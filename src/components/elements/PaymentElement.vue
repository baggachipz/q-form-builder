<template>
  <div class="q-form-payment q-form-builder-element">
    ...
    <div class="row q-col-gutter-sm">
      <div class="col-12 col-md-6"><q-input v-model="innerValue.ccnum" label="Card number" mask="################" :debounce="debounce" :rules="getRules('ccnum', required)" :ref="`${id}_ccnum`" /></div>
    </div>
    <div class="row q-col-gutter-sm">
    ...
    <div class="row q-col-gutter-sm">
      <div class="col-12 col-sm-6 col-md-3"><q-input v-model="innerValue.cvv" label="CVV" hint="3 or 4 digit code on back of card" mask="####" :debounce="debounce" :rules="getRules('cvv', required)" :ref="`${id}_cvv`" /></div>
    </div>
...
export default {
  ...
  computed: {
    ...
    rules () {
      return {
        'ccnum': [val => luhn.validate(val) || 'Please enter a valid credit card.']
      }
    }
  }
}
